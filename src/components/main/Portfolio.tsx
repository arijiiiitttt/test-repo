...
// **FIX for TS2322:** The return type is updated to correctly be 'HTMLAnchorElement | null'
function useScrollFadeUp(): [React.RefObject<HTMLAnchorElement | null>, boolean] {
...
export default function Profile(): JSX.Element {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setWave((prevWave) => (prevWave + 1) % 4);
    }, 300);
    return () => clearInterval(intervalId);
  }, []);

  const waveEmojis = ['👋🏼', '🖐🏼', '🖖🏼', '✋🏼'];
  const getAnimatedWave = () => waveEmojis[wave];
  ...
  const links: LinkItem[] = [
    ...
    },
    {
      ...
      image: '',
      description:
        'Tracks the latest available hackathons and competitions',
      video: '',
      live: false,
    },
    {
  ...
  return (
    ...
    >
      ...
      <div className="pt-16 px-4 flex flex-col items-center">
        <h1 className="md:text-4xl lg:text-4xl text-3xl font-bold text-center pl-7 tracking-tighter">
          Hi, I'm Arijit Roy {getAnimatedWave()}
        </h1>
        <p
          className={`mb-2 text-[13px] mt-1 text-center ${
                    ...
                    <span
                      className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 ${
                        theme === 'dark'
                          ? 'border-gray-950'
                          : 'border-white/80'
                      } ${link.live ? 'bg-green-500' : 'bg-red-500'}`}
        ...
        {/* Contact */}
        <div
          className={`w-full max-w-lg mt-14 px-6 py-8 ${
            theme === 'dark'
        ...
        >
          ...
          >
            Whether it's a collab, a question, or just a friendly hello, drop me
